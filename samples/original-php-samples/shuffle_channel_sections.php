...
if ($client->getAccessToken ()) {
	try {
		...
		$listResponse = $youtube->channelSections->listChannelSections ( 'snippet,contentDetails', array (
				'mine' => true 
		) );
		...
		foreach ( $channelSections as $channelSection ) {
			...
			$htmlBody .= sprintf ( '<li>%s "%s"</li>', $updateResponse ['id'], $updateResponse ['snippet'] ['title'] );
		}
